...
iRPCClusterContainer iRPCClusterizer::doAction(const RPCDigiCollection::Range& digiRange, iRPCInfo& info)
{
  ...
  //std::cout <<"strip=" << digi->strip() << " time=" <<  digi->time() << " position=" << digi->coordinateY() << std::endl;
  //}
  ...
  iRPCHitContainer hr, lr;
	for(auto digi = digiRange.first; digi != digiRange.second; ++digi) {
      ...
      hr.push_back(iRPCHit(ch, strip, timeHR, digi->bx())); hr.back().setHR(true);
     if(digi->hasY()) {  // for tests
      float timeLR = timeHR-digi->coordinateY()/info.speed();
      lr.push_back(iRPCHit(ch, strip, timeLR, digi->bx())); lr.back().setLR(true);
     }
  }
	
  // Clustering for HR and LR separate.
	iRPCClusterContainer chr, clr;
 	std::thread thr (&iRPCClusterizer::clustering, this, info.thrTimeHR(), std::ref(hr), std::ref(chr));
 	std::thread tlr (&iRPCClusterizer::clustering, this, info.thrTimeLR(), std::ref(lr), std::ref(clr));
